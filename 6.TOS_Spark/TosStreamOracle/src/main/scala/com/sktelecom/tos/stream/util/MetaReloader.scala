...
object MetaReloader extends Logging {
  ...
  val usage =
    """
       Usage: MetaReloader  [--zookeeper <zookeeper> --znode <znode>] | [ [--host <host>] --port <port> [--object-name <object name>] ]
      """.stripMargin
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      process(host, port, objectName + ":type=meta")
    } catch {
  ...
  def process(host: String, port: Int, objectName: String) {
    ...
    try {
      ...
      val mbeanProxy = JMX.newMBeanProxy(mbsc, mbeanName, classOf[MetaSuperManagerMBean])
      mbeanProxy.reload()
      logInfo("reloaded the meta")
    } catch {
