...
/**
  * URT log 를 수신하여 정제 및 매핑하여
  * CEP Topic 및 Save Topic에 전송한다
  *
  * @param configManager tos.stream의 Config를 관리하는 클래스
  */
class URTStreamApp(configManager: ConfigManager) extends TosStreamApp with Serializable {
...
object URTStreamApp {
  def main(args: Array[String]): Unit = {
    new URTStreamApp(ConfigManager(getClass.getSimpleName.stripSuffix("$"))).start
  }
}